...
2. Run: timeout 540 bash ${a.plugin_root}/skills/use-${name === 'carl' ? 'gemini' : 'codex'}/scripts/${script} -f "${promptFile}" -o "${outFile}" (the timeout prefix is mandatory: a helper that outlives the Bash cap gets backgrounded and stalls this lane). Note the exit code and stderr.
3. If the helper is missing, exits non-zero without output, exit code 124 (timed out after 540 s), or exit code 4 (permanently unavailable), return lane_status unavailable/failed with the reason in status_detail and no findings. Otherwise Read ${outFile} and translate every issue line of the form "[NAME] <emoji> <title> | File: <file> | Task: <task>" into a finding (🔴 CRITICAL, 🟠 HIGH, 🟡 MEDIUM, ⚪ LOW; evidence = the line's own text plus any explanation that follows it; proof = the reviewer's stated failing input/consequence when given, else empty), and copy the R-rule verdict lines verbatim into verdict_lines. raw_excerpt = the first 4000 chars of the output.
