...
- **Always run review phases in a fresh session.** When `/run-autopilot`'s Phase 3 (Work) completes, the agent must NOT continue into Phase 4 in the same session — the review phases (4, 7, 8) each spawn multiple cloud reviewers and need a clean context window. Hand off by ending the turn: set `state.next_phase` in `dev/local/autopilot/state.json`, print the handoff banner, then STOP. In loop mode each session is headless (`claude -p`) and exits at turn end; the `autoclaude` wrapper reads `state.json` and relaunches `/run-autopilot` in a fresh session, which resumes at the recorded phase. Never ask the user to press Ctrl+D — the process exits on its own. Never invoke `/review-work-completion` from the work session, even if context budget appears sufficient.
