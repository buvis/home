...
`~/.local/share/agents/cartographer/projects/`. The aggregation is deterministic, so a
...
- Paths: `~/.local/share/agents/cartographer/projects/<hash>/atlas.json` (written by
  `survey` plus the cartographer hooks), `~/.local/share/agents/instincts/projects.json`
...
`~/.local/share/agents/instincts/projects.json` for repos active in the last 30 days) and
...
- Atlas dir: `~/.local/share/agents/cartographer/projects/<hash>/`; schema keys:
