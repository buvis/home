...
1. **Regroup commits produced by this PRD.** Operate on the range `<work_start_sha>..HEAD` (where `<work_start_sha>` is `state.work_start_sha`). **This step runs unconditionally — do NOT gate it on `deferred_decisions` being empty or `doubts` being resolved.** Run the sub-behaviors below in order; emit exactly **one** outcome line per the five shapes documented in `references/batch-report-format.md` (Regroup Outcome). Write the chosen outcome line to `state.regroup_outcome` (see `references/state-schema.md`) so step 7 can include it in the batch report.
