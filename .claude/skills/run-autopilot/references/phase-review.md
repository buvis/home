...
Do NOT rewrite `state.tasks` here — `/work` already wrote `attempts[]` entries directly to `state.tasks` during rework, and a bare TaskList snapshot would strip them; the sync hook keeps `tasks_total`/`tasks_completed` current. `phase-done` does not touch `state.tasks` at all, which is why the snapshot cannot be lost by advancing the cycle.
