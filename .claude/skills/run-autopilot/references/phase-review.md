...
Apply steps 1-3 with `statectl set` (one call per field — four field-writes across the three steps; each write is individually atomic), immediately before the banner and turn-end. These are separate non-transactional writes, so the ORDER matters. The load-bearing durable write is step 2's `cycle` increment: it MUST land, because the Phase 5 cap gate reads `state.cycle` (step 2 spells out why). The step 3 `phase`/`next_phase` re-affirmations are belt-and-suspenders — both are already `"review"` throughout the review gate, so losing *those specifically* cannot misroute the phase. But do not over-read that into a blanket safety claim: a crash after step 1 and before step 2's increment lands is not free — the fresh session re-enters the *same* (un-incremented) cycle (its review file already exists, so Phase 4 skips re-review and Phase 5 re-runs), a recoverable, redundant cycle rather than a silent cap miss. A transactional/batch verb landing all the writes atomically — which would close that crash window — is out of scope here (owned by PRD 00051's writer boundary):
