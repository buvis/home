...
```json
{
  ...
  "tasks": [
    {"id": "task-uuid-1", "name": "Add validation endpoint", "status": "completed", "model": "sonnet", "attempts": [
      {"attempt": 1, "model": "sonnet", "outcome": "completed", "review_cycle": null, "cause": null, "implementor": "claude", "preflight_outcome": null, "self_deslop": "noop"}
    ]},
...
| `next_phase` | string | Phase the next iteration of `/run-autopilot` will run. Written by `/run-autopilot` immediately before the `signal` file. Read by `autoclaude` (`_autoclaude_pick_model`) to pick the launch model: `"work"` → Sonnet (subject to `launch_model` override), every other value → Opus 1M. The authoritative resume signal is `phase` + `phases_completed`. |
| `launch_model` | enum? | Work-session launch tier: `"haiku"`, `"sonnet"`, `"opus"`. Written by the Phase 2→3 handoff (always explicitly: `"opus"` when PRD frontmatter `default_model: opus` or `replan_count >= 1`, else `"sonnet"`). Read by `_autoclaude_pick_model` ONLY when `next_phase == "work"` — non-work launches ignore it, so a stale value can never demote a review session. Cleared defensively by Phase 9 step 10. Absent → work launches on Sonnet. |
...
| `context_window` | int? | Context window (tokens) of the model the current session launched on. Written by `autoclaude` (`_autoclaude_pick_model`) before each launch: `1000000` for Opus launches, `200000` for plain Sonnet/Haiku work launches (the `[1m]` Sonnet variant bills API credits and is never used). Read by `autopilot_context_cap_hook.py` to size the Work-phase context cap — window ≥ 400000 → 500K cap, otherwise 150K. A Sonnet work session therefore runs under the 150K cap; a cap overrun replans and the relaunch escalates to Opus via `launch_model`. Absent → hook assumes the 150K standard-tier cap (conservative). |
...
| `cap_pause_reason` | object? | Set by Phase 5 when the rework cap is hit AND the current review did not converge (`cycle >= rework_cap` AND unresolved findings remain). A clean cycle-at-cap convergence does NOT trigger cap-pause — it falls through to Phase 7 hand-off. Records the unresolved review findings and the cycle count. Shape: `{"cycle": int, "cap": int, "unresolved_findings": object[]}`. **Distinct from `stall_reason`'s `oversized_task`, `context_overrun`, `subagent_prompt_overrun`, and `escalation_exhausted` shapes** — `cap_pause_reason` is a separate top-level state field, not another `stall_reason` discriminator. The authoritative, durable indicator of a cap pause is `phase == "paused"` PLUS this field — not `needs_attention`. Cleared by the Phase 0 Cap-Pause Resume Handler on user "resume". Per-PRD scope: a cap pause halts the flow before Phase 9, so Phase 9 step 10 never observes this field in normal operation and no explicit clear is needed. The only residual-leakage path is a manual "abandon" followed by a manual PRD move, which falls outside the automated flow. Absent in normal operation. |
