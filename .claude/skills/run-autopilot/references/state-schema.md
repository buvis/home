...
| `next_phase` | string | Phase the next iteration of `/run-autopilot` will run. Written by `/run-autopilot` immediately before the `signal` file. Kept only for `autoclaude`'s debug log — every session now launches on Opus 1M regardless of phase (`_autoclaude_pick_model`). The authoritative resume signal is `phase` + `phases_completed`. |
...
| `context_window` | int? | Context window (tokens) of the model the current session launched on. Written by `autoclaude` (`_autoclaude_pick_model`) before each launch: now always `1000000` (every session runs on Opus 1M). Read by `autopilot_context_cap_hook.py` to size the Work-phase context cap — window ≥ 400000 → 500K cap, otherwise 150K. The always-1M value yields a 500K cap, which removed the 150K Sonnet-window livelock. Absent → hook assumes the 150K standard-tier cap (conservative). |
