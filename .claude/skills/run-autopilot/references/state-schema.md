...
| `phase` | enum | Current gate: `build`, `review`, `blind`, `doubt`, `done`, `paused`. `build` is ONE session (selection → catchup → planning → work, no mid-build handoff); the three review surfaces (`review`, `blind`, `doubt`) each run in a fresh session. |
