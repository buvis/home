...
| `tasks[].estimated_tokens` | int? | Per-task input-token budget estimate. Set by plan-tasks step 4.7's tier classifier; carried across sessions by the State Management hydration sub-step. Absent when plan-tasks did not record a budget estimate. |
| `tasks[].est_context_peak` | int? | Per-task estimated peak working-context size. Used to predict subagent dispatch budget headroom. Set by plan-tasks step 4.7's tier classifier; carried across sessions by the State Management hydration sub-step. Absent when plan-tasks did not record a context estimate. |
| `tasks[].qwen_eligible` | bool? | Qwen routing eligibility (PRD 00032, widened by PRD 00019): backend (not UI) AND tier ∈ {`haiku`, `sonnet`} AND `files_touched <= 3` AND no public-contract edit. Set by plan-tasks step 4.7's tier classifier; carried across sessions by the hydration sub-step. `/work` reads this field for routing; absent (legacy plans) is treated as `false`. |
| `tasks[].qwen_excluded_reason` | string? | Why plan-tasks step 4.7 excluded the task from qwen routing: `"ui"`, `"tier"`, `"files"`, or `"contract"` (first failing condition in that order). Set by plan-tasks step 4.7's tier classifier on every ineligible task (PRD 00019); carried across sessions by the hydration sub-step. Absent on eligible tasks and legacy plans — the Phase 9 Implementor Mix render counts an ineligible task with no reason as `unknown`, never an error. |
