...
7. Append the PRD summary to the batch report under the **report-id invariant** (core `SKILL.md` § "Phase 9 invariants"): the filename is built from the current `state.batch.id` — `dev/local/autopilot/reports/{state.batch.id}-report.md` (create with header if missing), never chosen by glob. See `references/batch-report-format.md` for format. Render these subsections:
   ...
   - **Implementor Mix (PRD 00019):** render an `### Implementor Mix` subsection from `state.tasks[]` — (1) count attempts by `attempts[].implementor` (`claude`/`qwen`/`gemini`; an attempt row without the field counts as `unknown`); (2) list qwen preflight outcomes by counting non-null `attempts[].preflight_outcome` values per value; (3) count ineligible tasks per `qwen_excluded_reason` (tasks whose `qwen_eligible` is `false` or absent; a missing plan-time reason counts as `unknown`), plus — additionally bucketed as `memory_pressure` and `memory_probe_failed` (PRD 00075) — every task with **at least one** attempt carrying that `qwen_excluded_reason`, **deduplicated by task id** so each bucket counts tasks, not attempts; an absent attempt-level `qwen_excluded_reason` contributes to no bucket (it means no memory-pressure reroute occurred, not `unknown`); these two buckets do NOT partition the plan-time-ineligible set the other four draw from (a memory-pressure task was `qwen_eligible == true` and routed off qwen at dispatch, not excluded at plan time — see `references/batch-report-format.md` for the full rule). Absent `implementor` or plan-time-reason fields (legacy tasks) always count as `unknown` (the attempt-level `qwen_excluded_reason` above is the one exception, per the note just given); the render never fails the report — when `state.tasks[]` is missing or empty, render `no implementor data` instead.
