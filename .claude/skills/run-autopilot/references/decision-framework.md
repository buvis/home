...
2. **Stall the PRD, continue the batch** — for cap-out CRITICALs, exhausted retries (sub-skill/reviewer re-invoked once and still failing, replan cap exhausted), and forbidden-assumption ambiguities: follow `recovery.md` → "Loop-mode stall procedure" (move to `hold/`, record `{"type": "stall", "site", "detail", "prd"}` in the deferred JSON, advance to the next PRD).
