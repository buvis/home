...
| 2 | `haiku` | `files_touched ≤ 2` AND est. lines-changed `≤ 50` AND task text matches a mechanical-pattern signal (`add log`, `rename`, `add test for`, `port`, `mirror`, `inline`, `extract constant`, `update import`, `bump version`, `wire` (whole word), `disable`, `restore`, `pin the`) AND no Rule 1 keywords present |
| 3 | default | `sonnet`, unless PRD frontmatter `default_model:` is set (see override below) |

**Rule 2 signal-widening rationale (PRD 00075 evidence pass, `dev/local/audit-results/00075-task-mix-evidence.md`).** `disable` and `restore` are kept as bare (substring) words: their only substring collisions are same-meaning inflections (`disabled`/`disability`, `restored`/`restoration`), so the over-match risk is negligible. `wire` is kept, but it is NOT matched as a bare substring: match it only as a **whole word** (word-boundary match, e.g. `\bwire\b`), because as a bare substring it collides with unrelated words — `wireless`, `wireframe` — that have nothing to do with wiring one thing into another (e.g. "Add wireless OTA firmware update capability to the sensor driver" would wrongly hit `haiku` under bare-substring matching). The earlier claim that `wire` "has no meaningful unrelated collision" was wrong; the whole-word restriction is what keeps it safe. `pin` is bound to the phrase `pin the` because the bare word collides with unrelated common words (`mapping`, `opinion`, `spinning`) that share no mechanical-edit meaning. `permission` was proposed and briefly shipped as a bare word, but it is dropped: unlike `disable`/`restore`, its substring collisions are not limited to same-meaning inflections — it appears throughout substantive authorization/access-control feature work (e.g. "Add role-based permission checks to the billing API", 2 files, ~45 lines, no Rule 1 keyword, would wrongly route security-sensitive work to `haiku`), so the over-match risk is real. Dropping `permission` does not break the evidence bar: the "single config-file entry addition" shape it was meant to catch still has two git-verified backing instances without it — 00064-task-2 (matched by `wire`) and 00072-task-8 (matched by `disable`). The evidence pass also proposed `complete`, but it is dropped: `complete the <X>` is generic English for "finish X" and does not specifically denote a small mechanical edit (e.g. "complete the retry branch in auth.py" reads identically for trivial and substantive work), and no contiguous-phrase binding removes that ambiguity without becoming too narrow to generalize.
