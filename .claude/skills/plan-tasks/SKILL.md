...
**Pending re-derivation (PRD 00084 R3, LIVE):** the 55K/150K constants are measured on `opus-4.8`, but PRD 00076 ended Opus as the default autopilot **build**-phase model: build sessions now launch Sonnet-first and promote to `claude-opus-5` only when `_autopilot_build_model` finds difficulty evidence for the PRD. `_AUTOPILOT_MODEL_BUILD` is an operator env override / kill-switch, not a hardcoded constant. That makes this trigger live: re-derive per the mandate above on real Sonnet build transcripts once a few Sonnet build batches exist, not from a single ad-hoc session or a non-build transcript.
