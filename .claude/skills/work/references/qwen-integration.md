...
The preflight runs once per task attempt, except when the qwen capability breaker (row 3) or the memory-pressure gate (row 4) reroutes a qwen-eligible task to Claude before the probe runs, skipping it entirely. It does NOT run on Claude or Gemini dispatches.
