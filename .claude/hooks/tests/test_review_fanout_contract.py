...
Lives here, not in the autopilot pack, because the workflow is host-local:
`review-work-completion/SKILL.md` declares it "not shipped with this pack",
so the pack must not carry a test that hard-fails without it. `hooks/tests/`
is this config directory's pytest home (`test_settings_wiring.py`,
`test_survey.py` and friends are no more hook tests than this one). The one
thing it still needs from the pack, `check_review_file.py`, is resolved from
the installed plugin at call time.
...
def _workflows_dir() -> Path:
    """The workflows/ directory holding the engine this file pins.

    Found by walking up for the harness itself, never by a `parents[N]` count:
    this file used to live in the autopilot pack, where `parents[3]` meant
    `~/.claude`, and the 2026-08-25 plugin extraction re-pointed that same
    expression at the plugin repo root without a word. All 18 cases errored
    from then until PRD 00158 moved the file here, beside what it pins.
    """
    for candidate in Path(__file__).resolve().parents:
        harness = candidate / "workflows" / "_harness.mjs"
        if harness.is_file():
            return harness.parent
    raise RuntimeError(
        "no workflows/_harness.mjs above "
        f"{Path(__file__).resolve()}; this file must live under the config "
        "directory that holds the review-fanout workflow it pins.",
    )


WORKFLOWS_DIR = _workflows_dir()
WORKFLOW_JS = WORKFLOWS_DIR / "review-fanout.workflow.js"
HARNESS_MJS = WORKFLOWS_DIR / "_harness.mjs"


def _check_review_file_py() -> Path:
    """`check_review_file.py`, which belongs to the autopilot plugin, not here.

    Same resolution order as `_autopilot_skill_root` in
    `~/.config/bash/plugins/development.plugin.bash`: the `_AUTOPILOT_SKILL_ROOT`
    override (it names `<root>/skills/run-autopilot`, so the pack root is two
    levels up), then the highest installed version in the plugin cache. Never a
    pinned version - the cache rolled 0.1.1 -> 0.1.2 under a running PRD once and
    stranded the driver that had pinned it. Resolved at call time so a missing
    plugin fails only the three cases that need it.
    """
    override = os.environ.get(PLUGIN_ROOT_ENV)
    if override:
        gate = Path(override).resolve().parents[1] / GATE_RELATIVE
        if gate.is_file():
            return gate
        raise RuntimeError(
            f"{PLUGIN_ROOT_ENV}={override} names no {GATE_RELATIVE} "
            f"(looked for {gate}); point it at <checkout>/skills/run-autopilot.",
        )
    installed = [
        (_version_key(path.name), path / GATE_RELATIVE)
        for path in (PLUGIN_CACHE.iterdir() if PLUGIN_CACHE.is_dir() else [])
        if (path / GATE_RELATIVE).is_file()
    ]
    if installed:
        return max(installed)[1]
    raise RuntimeError(
        f"no {GATE_RELATIVE.name} found: no autopilot plugin under {PLUGIN_CACHE} "
        f"ships {GATE_RELATIVE}. Install the plugin, or set {PLUGIN_ROOT_ENV} to a "
        "checkout's skills/run-autopilot.",
    )


def _version_key(name: str) -> tuple[int, ...]:
    """Sort key for a cache directory name; unparsable names sort lowest."""
    try:
